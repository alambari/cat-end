    ...
    <a-col :span="24" :xl="16" class="mb-24">
      <a-form name="nest-messages" :model="state" @finish="onFinish" @finish-failed="onFinishFailed"
        ...
        <a-card :bordered="false" class="header-solid h-full">
          <a-form-item name="name" label="Nama Pemilik"
            :rules="[{ required: true, message: 'Nama pemilik harus diisi' }]">
            <a-input v-model:value="state.name" placeholder="Doe" />
          </a-form-item>
...
const fetchData = async () => {
  try {
    ...
    const { data, error } = await useFetch(`/api/pet-owner/show`, {
      params: {
        id: route.params.id
      }
    });
...
const onFinish = async (e: any) => {
  try {
    ...
    const { data, error } = await useFetch('/api/pet-owner/update', {
      method: 'PUT',
      body: {
        ...state,
        id: route.params.id,
    ...
    notification.success({
      message: 'Berhasil',
      description: item?.message ?? 'Pemilik berhasil di update'
    });
