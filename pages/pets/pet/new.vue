    ...
    <a-col :span="24" :xl="16" class="mb-24">
      <a-form name="nest-messages" :model="state" @finish="onFinish" @finish-failed="onFinishFailed"
        ...
        <a-card :bordered="false" class="header-solid h-full title-border-bottom" title="Pet">
          ...
          <a-form-item name="owner" label="Data Pemilik" :rules="[{ required: true }]">
            <a-select v-model:value="state.owner" placeholder="John" showSearch :filter-option="false"
              :not-found-content="owner.fetching ? undefined : null" :options="owner.data"
              :loading="owner.loading" @search="fetchOwner">
              <template v-if="owner.fetching" #notFoundContent>
                <a-spin size="small" />
              </template>
            </a-select>
          </a-form-item>
...
const fetchOwner = debounce(async (value: any) => {
  lastFetchId += 1;
  ...
  owner.data = dataList.rows?.map((item: any) => ({
    label: item.name,
    value: item.id
  })) ?? [];
...
const onFinish = async (e: any) => {
  try {
    ...
    const { data, error } = await useFetch('/api/pets/create', {
      method: 'POST',
      body: {
        ...state,
        PetOwnerId: e.owner,
    ...
    notification.success({
      message: 'Berhasil',
      description: item?.message ?? 'Pet berhasil dibuat'
    });
